...
const IntroduceList = () => {
  ...
  const handleSaveClick = async (id) => {
    try {
      // API를 호출하여 제목 업데이트
      await api.patch("/interview/introduce/title", {
        interviewId: id,
        title: newTitle,
      });
  ...
  const handleDeleteClick = async (id) => {
    ...
    try {
      ...
      // 화면에 즉시 반영
      setVideos((prevVideos) =>
        prevVideos.filter((video) => video.interviewId !== id)
      );
    } catch (error) {
  ...
  return (
    <div className="video-container">
      <h3 className="video-count">
        총 {videos ? videos.length : 0}개의 면접 결과
      </h3>
            ...
            <div key={video.interviewId} className="video-item">
              ...
              {editingId === video.interviewId ? (
                <div className="title-edit-container">
                  ...
                  />
                  <button
                    onClick={() => handleSaveClick(video.interviewId)}
                    className="save-btn"
                  >
                    저장
                  </button>
                ...
                <div className="title-container">
                  ...
                  <span
                    className="edit-icon"
                    onClick={() =>
                      handleEditClick(video.interviewId, video.title)
                    }
